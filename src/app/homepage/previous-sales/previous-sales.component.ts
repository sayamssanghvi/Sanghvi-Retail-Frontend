...
export class PreviousSalesComponent implements OnInit {
  ...
  GetSales(): void {
    ...
    this.customerService
      .GetSalesByPhoneNumber(this.phoneNumber.value, options)
      .subscribe(
        (res: any) => {
          if (res.status == 1) this.sales = res.sales;
        },
        (error: any) => {
          console.log(error);
          if (error.status >= 400) this.openSnackBar(error.error.error, 5000);
          if (error.status >= 500) this.openSnackBar(error.error.error, 5000);
        }
      )
      .add(() => {
        this.enableButtons();
  ...
  GetRepairSales(): void {
    ...
    let options = { sort: 'createdAt', order: 'desc' };
    this.customerService
      .GetRepairMachineByPhoneNumber(this.phoneNumber.value, options)
      .subscribe(
        (res: any) => {
          if (res.status == 1) {
  ...
  OnRepairClick(repair: any): void {
    this.repairDialog.open(RepairDetailDialogComponent, {
      data: { ...repair, customerName: this.customerName },
      width: '1000px',
      height: '400px',
  ...
  private disableButtons(): void {
    ...
    this.renderer.setProperty(
      this.repairButton.nativeElement,
      'disabled',
      true
    );
  }
  ...
  private enableButtons(): void {
    this.renderer.setProperty(
      this.salesButton.nativeElement,
      'disabled',
      false
    );
    this.renderer.setProperty(
